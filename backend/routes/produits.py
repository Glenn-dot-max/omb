...
async def create_produit(produit: ProduitCreate, current_user: dict = Depends(get_current_user)):
    ...
    logger.info(f"🆕 Creating Produit: {produit_data}")
    ...
    if current_user.get("role") in CATALOG_ADMIN_ROLES:
        ...
        if reusable_produit_id:
            ...
            logger.info(f"✅ FRANCHISE : Produit réactivé et mis à jour ({reusable_produit_id})")
    ...
    for franchise_id in franchise_ids:
        try:
            ...
            logger.warning(f"⚠️ Erreur ajout franchise {franchise_id}: {str(e)}")

    logger.info(f"✅ Produit créé et activé pour {liens_crees} franchise(s)")
...
async def delete_produit(produit_id: str, current_user: dict = Depends(get_current_user)):
    ...
    logger.info(f"🗑️ DELETE REQUEST - Produit ID: {produit_id}")
    ...
    if current_user.get("role") in CATALOG_ADMIN_ROLES:
        logger.info(f"✅ TECH_ADMIN : Suppression complète du produit {produit_id}")
        ...
        try:
            ...
            logger.info(f"✅ Produit supprimé complètement : {produit_id}")
    ...
    # 🔒 FRANCHISE : désactivation uniquement pour sa franchise
        ...
        try:
            ...
            if is_franchise_owned:
                ...
                logger.info(f"✅ FRANCHISE : Copie produit supprimée définitivement {produit_id}")
            ...
            logger.info(f"✅ FRANCHISE : Produit désactivé pour la franchise {franchise_id}")
...
async def toggle_produit_franchises(produit_id: str, request: ToggleFranchisesRequest, current_user: dict = Depends(get_current_user)):
    ...
    logger.info(f"🔄 TECH_ADMIN : {'Activation' if request.active else 'Désactivation'} du produit {produit_id} pour les franchises {request.franchise_ids}")
...
async def update_produit(produit_id: str, produit: ProduitUpdate, current_user: dict = Depends(get_current_user)):
    ...
    logger.info(f"✏️ UPDATE REQUEST - Produit ID: {produit_id}")
    ...
    if current_user.get("role") in CATALOG_ADMIN_ROLES:
        ...
        logger.info(f"✅ Produit modifié : {produit_id}")
        return response.data[0]
    ...
    if len(liens.data) > 1:
        logger.warning("⚠️ FRANCHISE : Produit partagé - Création d'une copie exclusive")
        ...
        logger.info(f"✅ FRANCHISE : Copie exclusive créée et activée ({new_produit_id})")
    ...
    logger.info(f"✅ FRANCHISE : Produit exclusif modifié {produit_id}")
    return response.data[0]
